const HEADERS = ["", "askaisl", "Human moderator", "Survey form"];
...
export default function ComparisonTable() {
  return (
    <section style={{ maxWidth: 1200, margin: "0 auto", padding: "100px 52px" }}>
      <div style={{ fontSize: 12, color: "var(--txt3)", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 600, marginBottom: 20 }}>Why askaisl</div>
      ...
      <div style={{ marginTop: 56, border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden" }}>
        {/* Header row */}
        <div className="grid grid-cols-[2fr_1fr_1fr_1fr] bg-[var(--bg2)] border-b" style={{ borderColor: "var(--border)" }}>
          {HEADERS.map((h, i) => (
        ...
        {ROWS.map(([feature, ...cells], ri) => (
          <div key={ri} className="vt-compare-row grid grid-cols-[2fr_1fr_1fr_1fr] border-b last:border-0" style={{ borderColor: "var(--border)" }}>
